...
- Advanced mode remains the full inspector/debugging surface, but now shares the same dark dotted Canvax design language, tighter deck sizing, sticky command header, mode explanation, mode guide, and deck labels so it reads as an advanced layer of the same product instead of a separate product. The guide maps Workbench to `Sketch` / `Talk` / `Make / Apply` and Advanced to `Project rail` / `Canvas deck` / `Handoff inspector`. The sticky command deck now uses a more opaque surface so scrolled canvas content does not visually bleed through it.
...
- `canvax-asset-candidates-latest.*` is exported as a prompt-ready image/asset candidate format with source frame, bounds, prompts, placement maps, and output slots.
