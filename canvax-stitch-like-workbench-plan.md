...
- Browser regression now has a `visualfixture=advanced-map` state that seeds a dense Map, switches to Advanced Flow view, scrolls under the command deck, captures desktop/tablet screenshots, and asserts the Advanced deck remains opaque while generated-output cards use designer-readable labels and frame rail output badges stay compact.
