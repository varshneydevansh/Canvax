...
- live preview tweak targeting where a selected output region becomes a Codex
  ...
  context plus a `codex-patch-task.json` handoff. `execute-patch` now applies
  that handoff to local Canvax-generated implementation bundles, the
  production-proof fixture, or explicit real project files listed in
  `exports/canvax-project-link-latest.json`; live DOM-semantic patching for
  arbitrary unlinked production apps and first-party host co-edit remain open.
...
- The Workbench composer now has a `Pin` action that turns the current typed or dictated instruction into a visible spatial Map note and voice-context entry. This keeps “what I meant while sketching” on the same canvas as frames, generated screens, files, references, and assets.
- Workbench quick refinement chips are now available both in the brief tray and above the floating scratchpad composer, keeping common prompts such as font, drama, mobile variant, spacing, and image candidates visible while designers work canvas-first.
- Workbench/Advanced `Map` now has a floating `Add to canvas` dock with Frame, Note, File, Group, Variants, Make, and Build actions. New frames, notes, groups, and reference files land in the currently viewed Map region, so designers can keep creating spatial objects and outputs from the canvas itself instead of returning to side-panel controls.
- Workbench and the Advanced left rail now share a browser-local project picker with `New`, `Duplicate`, `Delete`, and `Open project`. The shared `Browse` action opens a searchable Project Browser overlay with project cards, active state, frame counts, active-frame labels, and card-level Open / Duplicate / Delete actions. This gives Canvax a first Stitch-like project-switching layer while keeping the active project as the single live Codex handoff. Saves also write `exports/canvax-project-registry-latest.*` plus project-scoped latest handoffs under `exports/projects/<project-id>/` so the active board, asset/image-host handoffs, build request, and checkpoint latest/index files are recoverable outside volatile browser storage.
- The local `/api/preview-state` feed is now active-project-aware. Checkpoint history comes from the current project's checkpoint index, and generated output targets/artifacts/changes are filtered by project id or by frame ids in the current live export, so old Materialize/Build cards from another board do not reappear after switching projects.
- `scripts/write-codex-output.mjs` now resolves the active live-export project, attaches it to published output targets/artifacts/changes, and mirrors the same Codex output manifest to `exports/projects/<project-id>/canvax-codex-output-latest.{json,md}` while preserving the global compatibility manifest.
- Workbench `Map` now includes a spatial object layer: editable generated variant branches export through `spatialWorkspace.variantBranches`; labeled group regions, manual notes, reference files/images, asset candidates, generated screen targets, generated artifacts, changed files, and recent checkpoints become draggable object cards and export through `spatialWorkspace.objects`; selected Map objects can be wrapped into a group region, ungrouped without deleting contents, selected from group contents, fit back into group bounds, aligned with `Align L/C/R/T/M/B`, distributed with `Distribute X/Y`, copied/pasted through the system clipboard as spatial objects with remapped ids and visible placement, locked against accidental transform/reorder/align/distribute/duplicate/clipboard-copy/delete, sent backward or brought forward, and export `locked`, `layerIndex` / `layerLabel`; generated screen/file/change cards are grouped in a collapsible `Output shelf` lane with an inline legend and `Output ref` badge so they read as implementation outputs instead of extra frames, with old raw manifest labels and legacy materialized/generated-target records canonicalized into `Generated screen`, `Generated file`, and `Code change`; selected output/history cards can move earlier or later inside their lane and export through `meta.laneIndex`, context Markdown, and ordered `spatialWorkspace.lanes[].memberObjectIds`; selected variant/output-edit branch cards can move earlier or later in their source-frame branch sequence, and dragging branch cards across sibling branch positions updates `frame.variant.index` plus ordered `spatialWorkspace.variantBranches`; the compact `Map timeline` lets designers jump between frames, branches, outputs, and checkpoints, and exports the same sequence through `spatialWorkspace.timeline`; generated output previews can become editable `Output edit` frames with a flow connection, output-target metadata on the matching variant Map object, and explicit `outputEditBinding` in task/rewrite/build handoffs so Codex knows which generated target to revise; output correction marks export normalized changed-region bounds and output eraser gestures delete marks instead of saving eraser strokes; group regions move contained frame cards/objects while skipping locked child objects and containment is exported through `spatialWorkspace.groups` and `groupIds`; dragging cards or objects into the left/top edge expands workspace room while trailing space keeps the map from feeling bounded; the minimap navigator provides click-to-pan orientation, `Fit map` recovers visible frames and Map objects after zoom/pan or long-session clutter, and `spatialWorkspace.viewport` exports the current/last viewed Map region for Codex.
- Checkpoint cards now support `Replay as frame` from both the checkpoint panel and Map. Replay reads the saved checkpoint/export, creates a new editable `Checkpoint replay` branch from the active frame, uses the saved snapshot as an underlay, and preserves the source checkpoint/export path in frame notes. This gives designers a local branch-history affordance without pretending checkpoints are a full lossless vector-state restore.
...
- Browser regression now has `visualfixture=workbench-map` and `visualfixture=advanced-map` states. The Workbench fixture opens the designer-first Map focus on desktop and narrow widths, scrolls the spatial workspace into view, captures screenshots, and asserts the Map timeline, output shelf, spatial cards, first `Start with your design` chip, internal viewport height, and document-level overflow behavior. The default narrow Workbench smoke also fails if the current canvas is pushed below the first visible screen. The Advanced fixture seeds a dense Map, switches to Advanced Flow view, scrolls under the command deck, captures desktop/tablet screenshots, and asserts the Advanced deck remains opaque while generated-output cards use designer-readable labels and frame rail output badges stay compact.
...
- Preview now has `Mark tweak` for generated-output region corrections. Dragging over an output surface and entering a note writes `exports/canvax-preview-tweak-latest.*` plus an archived tweak record with frame, target, normalized bounds, pixel bounds, viewport, and note. `execute-rewrite-request` reads the matching latest tweak, adds it to affected-region/component-target context, and writes `codex-patch-task.json` with suggested selectors/files. `execute-patch` can apply that task to Canvax-generated implementation files, the production-proof fixture, or explicit files in the latest project-link contract while preserving patch metadata. This is local and no-API; arbitrary unlinked production app patching still needs Codex.
...
- Workbench now includes a compact lower-left `Agent log`. It aggregates recent Make/Apply/Review, rewrite, output activity, checkpoint, design-jury, and voice-intent events so the designer can confirm that Canvax registered an action or produced a new output without switching to Advanced diagnostics. The same activity is exported through `workbench.agentLog` and Build-with-Codex `implementationContext.workbench.agentLog`, so Codex can read the visible action history.
...
- `npm run inspect` now reads the latest live export, task pack, build request,
  rewrite request, project-link artifact, and Codex output manifest into a
...
- `npm run review-snapshot` now samples browser screenshots locally and writes a
  ...
  regression artifacts without claiming hosted AI visual critique.
- `npm run review-dom` now inspects the rendered Preview DOM/layout in local
  headless Chrome and writes a no-API review for overflow, offscreen elements,
  target sizes, headings, landmarks, motion cues, and Canvax source bindings.
...
- `canvax-image-host-task-latest.*` is exported as the no-API host execution contract for image generation: one task per candidate, prompt, placement contract, output slot, return instructions, acceptance criteria, and active Canvax project metadata.
- `canvax-image-results-latest.*` is now the no-API return contract for hosted image generation: returned files, URLs, or data images are bound back to candidate ids, slot ids, placement contracts, and project mirrors through `npm run import-image-results`.
...
- Workbench now shows saved asset candidates as compact cards; each candidate can place an editable image slot on its source frame/region, attach a generated image file or workspace image path back to that slot, preview attached candidates, select the placed image, copy a one-candidate host task, and accept one as the chosen output while preserving `assetCandidateId`. Accepted choices now export through `reviewSummary.acceptedCandidates`; `reviewSummary.groups` keeps a frame-grouped pending/placed/attached/accepted queue; `reviewSummary.hostHandoff` lists the no-API files and workflow for Codex/ChatGPT image hosts; each candidate includes a `placementMap` contract with normalized bounds, pixel bounds, CSS placement, target selector, and HTML slot scaffold for Codex/host image tools; and `canvax-image-host-task-latest.*` gives host tools a machine-readable generation/return checklist.
- Map now accepts direct file/image drops and creates a movable reference card at the pointer location, so screenshots, reference packs, PDFs, and returned image candidates can live on the spatial workbench without opening a side-panel picker.
- Map now accepts direct text paste/drop and creates movable note cards, so raw ideas, critique, prompt fragments, URLs, transcript snippets, and book/image notes can become canvas objects immediately.
...
- Self-test coverage includes tool rendering, drawing controls, select/move/resize, eraser layer behavior, Workbench/Advanced mode guide rendering, Workbench dock brush sizing, the Workbench Agent log and its export, default compressed output/history shelves, Workbench spatial map rendering/export for frames, group regions, selection-created groups, selecting group contents, fitting group bounds, ungrouping, group containment, direct file drop-to-Map placement, direct text paste/drop-to-Map placement, system clipboard copy/paste for Map objects, manual notes, single-object property editing and type details, Map object alignment/distribution, Map object layer ordering, asset candidates, generated targets, artifacts, changed files, checkpoint history lanes with collapse/expand state, opt-in materialized review aids, stroke-first semantic screen generation, flow links, selected-element prototype hotspots, task/rewrite/image prompt packs, asset candidate packs, materialize, output activity, rewrite queue, and large-session export consistency.

Still open:

- automatic host image generation remains open. Multi-candidate review now has per-candidate prompt/placement copy, local attached-image thumbnails, workspace-path import, no-API hosted-result import, style-lock continuity metadata, select, and accept state in the candidate tray.
- true infinite spatial canvas beyond the Workbench Map frame/variant/context/generated-output/checkpoint object layer, especially fuller nested object modeling and richer schema-specific property panels beyond the current custom `key: value` layer. Generated output objects are now reconciled, grouped in the collapsible `Output shelf`, start compressed with checkpoint history for new/migrated sessions, can be cleared from Map, infer frame binding from current and legacy generated-output paths, canonicalize raw labels into designer-facing `Generated screen` / `Generated file` / `Code change` labels with an `Output ref` badge, and legacy stale/deleted-frame cards are cleaned up to reduce materialized-output clutter; the history lane can also be collapsed/expanded, Map objects can be focused by output/assets/notes/history, important objects can be pinned across those focus states, selected Map objects can be locked against accidental transform/reorder/align/distribute/delete, grouped/ungrouped, selected from group contents, fit into group bounds, aligned, distributed, recursively move/resize geometry-contained nested group contents, exported with parent/child group paths through `spatialWorkspace.groupHierarchy`, sent backward/brought forward, moved earlier/later inside output/history lanes, moved earlier/later inside branch sequences, drag-reordered by Map position with visible drop targets, edited with object-level Prompt / Context guidance and custom properties, navigated through frame/branch/output/checkpoint `Map timeline` tracks, and reflowed with `Tidy map`, and all of that exports for Codex.
...
- **Acceptance Criteria**:
  ...
  - Exports image host task files for hosted image generation without requiring Canvax to call an API.
  - Imports hosted image result files/URLs/data images back into candidate output slots without requiring Canvax to call an API.
...
- **Acceptance Criteria**:
  ...
  - On narrow windows, the current canvas appears before the long brief/project tray. **Covered by browser responsive smoke.**
...
- **Validation**:
  ...
  - Browser screenshots at 1440, 1024, 768, 430 widths. **Covered structurally for board/Preview plus Workbench Map desktop/narrow and Advanced Map desktop/tablet; still manual/open for visual taste review.**

### Task 6.2: Browser Regression Matrix

- **Status**: Expanded browser and lifecycle matrix shipped. Board self-test now includes a long-session Map stress fixture with many captured frames, voice notes, asset candidates, generated screen targets, artifacts, changed files, and checkpoint cards; responsive board/Preview smoke runs at 1440, 1024, 768, and 430 widths; Workbench Map visual smoke captures desktop/narrow screenshots and checks the Map timeline/output shelf/spatial-card layout; Advanced Map visual smoke captures desktop/tablet screenshots and checks the dense inspector state; runtime status validation checks PID/workspace/runtime identity before reuse; isolated lifecycle regression covers start/reuse/port-mismatch/restart/stop on throwaway ports. Non-Canvax occupied-port recovery remains open.
...
- create a Codex-readable task artifact for the active frame/checkpoint **done as `exports/canvax-build-real-latest.*`, `exports/projects/<project-id>/canvax-build-real-latest.*`, and `artifacts/canvax/build-requests/`**
