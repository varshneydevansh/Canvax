...
- Workbench `Map` now includes a first spatial object layer: asset candidates become draggable object cards and export through `spatialWorkspace.objects`.
...
- true infinite spatial canvas beyond the initial Workbench Map frame/variant/object layer
...
- **Status**: Initial version shipped. Workbench now has a `Map` focus that exposes the frame/variant graph plus asset candidate objects as a zoomable spatial project map and exports it as `spatialWorkspace`. This is not yet a true arbitrary-object infinite canvas.
- **Location**: `web/app.js`, `web/styles.css`
...
- **Validation**:
  ...
  - Board self-test verifies Workbench Map renders, zooms, and exports frames plus spatial objects.
