...
- true infinite spatial canvas beyond the Workbench Map frame/variant/context/generated-output/checkpoint object layer, especially richer nested group editing, editable per-type object inspectors, and history-lane reorder/filter/pin controls. Generated output objects are now reconciled, can be cleared from Map, and legacy stale cards are cleaned up to reduce materialized-output clutter; the history lane can now be collapsed/expanded and exports that state, but full timeline editing remains open.
