...
- Workbench `Map` single-object selection includes a lightweight Title/Note/Status property editor, with manual overrides preserved when generated/asset/checkpoint objects resync from handoff files.
...
- Self-test coverage includes tool rendering, drawing controls, select/move/resize, eraser layer behavior, Workbench dock brush sizing, Workbench spatial map rendering/export for frames, group regions, group containment, manual notes, single-object property editing, asset candidates, generated targets, artifacts, changed files, checkpoint history lanes, opt-in materialized review aids, flow links, selected-element prototype hotspots, task/rewrite/image prompt packs, asset candidate packs, materialize, output activity, rewrite queue, and large-session export consistency.
