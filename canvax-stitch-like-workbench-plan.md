...
- Workbench now includes a compact lower-left `Agent log`. It aggregates recent Make/Apply/Review, rewrite, output activity, checkpoint, design-jury, and voice-intent events so the designer can confirm that Canvax registered an action or produced a new output without switching to Advanced diagnostics.
