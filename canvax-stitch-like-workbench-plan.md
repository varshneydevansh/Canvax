...
- Workbench `Map` now includes a spatial object layer: asset candidates, generated preview targets, generated artifacts, and changed files become draggable object cards and export through `spatialWorkspace.objects`.
...
- Self-test coverage includes tool rendering, drawing controls, select/move/resize, eraser layer behavior, Workbench dock brush sizing, Workbench spatial map rendering/export for frames, asset candidates, generated targets, artifacts, and changed files, opt-in materialized review aids, flow links, task/image prompt packs, asset candidate packs, materialize, output activity, rewrite queue, and large-session export consistency.
...
- true infinite spatial canvas beyond the Workbench Map frame/variant/generated-output object layer
...
- **Validation**:
  ...
  - Board self-test verifies Workbench Map renders, zooms, and exports frames, asset candidates, generated targets, artifacts, and changed-file spatial objects.
