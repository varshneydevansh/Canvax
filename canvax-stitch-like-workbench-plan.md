...
- Workbench/Advanced `Map` now has a floating `Add to canvas` dock with Frame, Note, File, Group, Variants, Make, and Build actions, so designers can keep creating spatial objects and outputs from the canvas itself instead of returning to side-panel controls.
