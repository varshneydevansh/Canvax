...
- `canvax-image-host-task-latest.*` is exported as the no-API host execution contract for image generation: one task per candidate, prompt, placement contract, output slot, return instructions, and acceptance criteria.
