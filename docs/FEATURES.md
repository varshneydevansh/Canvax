...
flowchart LR
    ...
    Registry --> SnapA[Project snapshot A]
    Registry --> SnapB[Project snapshot B]
    SnapA --> Active[Active board]
    SnapB --> Active
    Active --> Export[exports/canvax-live-latest]
    Export --> Codex[Codex]
```
...
- exposes `Add context` in `More actions` so a designer can switch to Map and place a file/image reference card without starting from Advanced mode
...
- keeps the Live Edit dock visible in Workbench with `Pick`, action chip, pre-target rewrite note, variant counter, target-bound `Talk`, `+ Comment`, `Draw`, `Go`, `Accept`, and `Close` states, making point-and-mark editing the default loop instead of a hidden secondary mode
- lets Live Edit variants visually materialize inside picked canvas objects/regions and spatial Map cards with Structure/Taste/Clarity treatments, so non-DOM targets hot-swap in place while the user cycles variants instead of only showing a detached description
...
- lets designers select Map objects, Shift-click multi-select them, Shift-drag empty Map space to lasso-select them, drag a selected set together, resize the selected set from a combined transform box, see a visible Copy context/Pin/Lock/Group/Ungroup/Select contents/Fit group/Lane earlier/Lane later/Send back/Bring front/Duplicate/Delete/Clear action strip, copy a no-API Markdown object or selection handoff for Codex or image-generation prompting, copy/paste selected Map objects through the system clipboard as Canvax spatial objects with remapped ids and visible placement, nudge objects with arrow keys, group them with `Cmd/Ctrl+G`, ungroup selected group regions with `Shift+Cmd/Ctrl+G`, select group contents, fit group regions around their current contents, duplicate them with `Cmd/Ctrl+D`, reorder layers with `Cmd/Ctrl+[` and `Cmd/Ctrl+]`, move selected output/history cards earlier or later inside their lane, move selected variant/output-edit branch cards earlier or later in their source-frame branch sequence, drag branch cards across sibling branch positions to update branch sequence, delete them with `Delete`/`Backspace`, duplicate a group region with its contained Map objects, lock important references/outputs to block accidental move, resize, reorder, duplicate, clipboard-copy duplication, group, ungroup, lane-order, branch-order, or delete actions, and export the current pointed-at object/selection through `spatialWorkspace.selectedObjectId`, `spatialWorkspace.selectedObjectIds`, `spatialWorkspace.selectedObject`, `spatialWorkspace.selectedObjects`, per-object `prompt`, per-object `locked`, per-object `layerIndex` / `layerLabel`, lane order metadata, branch order metadata, and per-object `contextMarkdown`
- adds multi-selection alignment and distribution controls in `Map` (`Align L/C/R/T/M/B`, `Distribute X/Y`), so selected notes, frames, references, output cards, and group regions can be cleaned up directly on the spatial canvas without leaving the designer workflow
...
- adds a floating `Add to canvas` dock inside the spatial Map for one-click frame, note, file/reference, group, variant, generated-screen, and Codex build actions; new frame/context objects land in the visible Map viewport, making the Map feel like the primary workbench rather than a read-only flow view
...
- Repository design kits can be added as JSON files under `design-kits/`. They appear in the same searchable Design kit dropdown under `Repository kits`, export their source path in `designKit.preset.source`, and are validated by `npm run validate-design-kits` or discovered with `npm run validate-design-kits -- --query <term>`.
