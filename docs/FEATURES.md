...
- Repository design kits can be added as JSON files under `design-kits/`. They appear in the same Design kit dropdown under `Repository kits`, export their source path in `designKit.preset.source`, and are validated by `npm run validate-design-kits`.
