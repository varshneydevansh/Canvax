...
- `npm run extract-tokens` can scan public URLs, local HTML/CSS files, generated screen artifacts, pasted CSS/HTML text, or local raster screenshots for palette, CSS variables, font-family cues, and screenshot pixel-sample colors without an API key, writing `exports/canvax-external-design-tokens-latest.{json,md}`. Advanced `Import external` brings that latest pack back into the active Design kit.
