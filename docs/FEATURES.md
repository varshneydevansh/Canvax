...
- lets designers select Map objects, nudge them with arrow keys, duplicate them with `Cmd/Ctrl+D`, delete them with `Delete`/`Backspace`, duplicate a group region with its contained Map objects, and export the current pointed-at object through `spatialWorkspace.selectedObjectId`
