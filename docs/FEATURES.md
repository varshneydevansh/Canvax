...
- changes the dock exit to `Discard` while a temporary pick or variant set is active, and clears draft note/picker state on Escape or Discard so the restored original target does not keep stale edit residue
