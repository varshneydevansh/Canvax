...
- lets designers select Map objects, see a visible Copy context/Duplicate/Delete/Clear action strip, copy a no-API Markdown object handoff for Codex or image-generation prompting, nudge objects with arrow keys, duplicate them with `Cmd/Ctrl+D`, delete them with `Delete`/`Backspace`, duplicate a group region with its contained Map objects, and export the current pointed-at object through `spatialWorkspace.selectedObjectId`, `spatialWorkspace.selectedObject`, and per-object `contextMarkdown`
