...
- lets designers select Map objects, Shift-click multi-select them, Shift-drag empty Map space to lasso-select them, drag a selected set together, resize the selected set from a combined transform box, see a visible Copy context/Pin/Group/Ungroup/Send back/Bring front/Duplicate/Delete/Clear action strip, copy a no-API Markdown object or selection handoff for Codex or image-generation prompting, nudge objects with arrow keys, group them with `Cmd/Ctrl+G`, ungroup selected group regions with `Shift+Cmd/Ctrl+G`, duplicate them with `Cmd/Ctrl+D`, reorder them with `Cmd/Ctrl+[` and `Cmd/Ctrl+]`, delete them with `Delete`/`Backspace`, duplicate a group region with its contained Map objects, and export the current pointed-at object/selection through `spatialWorkspace.selectedObjectId`, `spatialWorkspace.selectedObjectIds`, `spatialWorkspace.selectedObject`, `spatialWorkspace.selectedObjects`, per-object `layerIndex` / `layerLabel`, and per-object `contextMarkdown`
