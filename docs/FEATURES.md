...
- `npm run extract-tokens` can scan public URLs, local HTML/CSS files, generated screen artifacts, or pasted CSS/HTML text for palette, CSS variables, and font-family cues without an API key, writing `exports/canvax-external-design-tokens-latest.{json,md}`. Advanced `Import external` brings that latest pack back into the active Design kit.
