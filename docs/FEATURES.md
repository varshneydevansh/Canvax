...
- lets designers turn an output preview card into an editable `Output edit` frame, preserving the source frame, generated target path, flow connection, variant branch object, and `spatialWorkspace.variantBranches[].outputBinding` so corrections can happen on a normal sketch frame instead of only on a passive output card
