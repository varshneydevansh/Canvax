...
- The preferred manual validation path is now the Codex in-app browser rather than a separate external browser: invoke `/canvax` to start or reuse the service and target `http://localhost:3210/?host=codex-sidecar` in the right-side in-app editor, open Preview, inspect generated routes, and publish output context back through the Codex output manifest. External browser use is explicit through `./canvax --open-external` or `./canvax --chrome`.
