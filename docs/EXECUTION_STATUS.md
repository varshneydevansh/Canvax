...
- [x] Workbench `Map` groups generated preview/file/code-change cards inside a named `Output shelf` lane so generated targets are readable as output references, not extra frames, and older raw manifest labels are canonicalized into `Output preview`, `Output file`, or `Code update`
