...
- [ ] Full interaction regression pass with long-session browser validation
- [ ] Remaining rough edges in large-session behavior

```text
done now:
  focus pad, board, tools, selection, editable image assets, flow, Workbench Map, manual context objects, readable generated output objects, optional live rewrite, preview button, cached frame renders, responsive smoke
still open:
  long-session browser validation, arbitrary-object infinite canvas, and long-session polish
```
