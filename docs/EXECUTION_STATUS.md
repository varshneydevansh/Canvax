...
- [x] Workbench `Open scratchpad` bottom designer dock with brush `-` / `+`, Image handoff, and tactile primary actions
- [x] Workbench quick-prompt chips add common refinement intent without opening Advanced mode
- [x] Workbench first quick chip is now `Start with your design`, which routes to Map and opens the local screenshot/file/image context picker
...
- [x] Workbench `More actions` now exposes `Add context`, which routes into Map and opens the same file/image reference-card picker used by Advanced
...
- [x] Browser regression now captures Workbench Map screenshots on desktop and narrow Codex/browser widths through `visualfixture=workbench-map`, and asserts the Map timeline, output shelf, spatial cards, first `Start with your design` chip, internal viewport height, and document-level overflow behavior
- [x] Browser regression now captures Advanced Map screenshots on desktop and tablet through `visualfixture=advanced-map`, and asserts the command deck stays opaque, frame output badges do not overflow the project rail, and no raw `generated-target` output labels are visible
- [x] Browser responsive smoke now fails on document-level horizontal overflow, covering the small-window Workbench project/action layout that previously looked clipped
- [x] Narrow Workbench now orders the current canvas before the long brief/project tray, and browser smoke fails if the canvas is pushed below the first visible screen again
- [x] Workbench `Map` spatial objects can be selected, Shift-click multi-selected, Shift-drag lasso-selected, moved as a selected set, resized as a selected set from a combined transform box, edited through a visible Copy context/Pin/Lock/Group/Ungroup/Select contents/Fit group/Align L/C/R/T/M/B/Distribute X/Y/Send back/Bring front/Duplicate/Delete/Clear action strip, copied as no-API Markdown context, keyboard-nudged, aligned, distributed, grouped with `Cmd/Ctrl+G`, ungrouped with `Shift+Cmd/Ctrl+G`, reordered with `Cmd/Ctrl+[` and `Cmd/Ctrl+]`, duplicated with `Cmd/Ctrl+D`, deleted with `Delete`/`Backspace`, locked against accidental move/resize/group/reorder/align/distribute/duplicate/delete, group-duplicated with contained unlocked Map objects, and exported as active `spatialWorkspace.selectedObjectId` / `selectedObjectIds` plus selected/per-object `locked`, `layerIndex`, `layerLabel`, and `contextMarkdown`
...
- [x] Workbench `Map` can copy and paste selected spatial objects through the system clipboard; Canvax writes a custom spatial-object payload, remaps pasted ids/group references into the visible Map region, strips lock/pin metadata through the manual-copy path, and falls back to plain Markdown context when the clipboard only preserves text
...
- [x] `npm run execute-patch` can patch explicit route/component/CSS files from the latest project-link contract when a `codex-patch-task.json` references those linked files, and can expand generated-output patch tasks to matching project-linked files through frame id + `data-canvax-node-id` bindings
- [x] Live Edit same-origin preview DOM picks preserve source/task hints such as `data-canvax-source-file`, `data-canvax-source-symbol`, and `data-canvax-task-id`, then thread accepted picks into rewrite/patch suggested files and component targets
- [x] `npm run execute-patch` can now apply explicit Live Edit source-hinted component/CSS files, mark the captured `targetTaskId` checklist row when one exists, and append hinted task notes without a project-link contract; regression covers that source-hinted patch path
- [x] Accepted output-backed Live Edit writebacks now save a post-writeback checkpoint that records rewrite execution, patch execution, changed files, and source-hint/project-link expansion status
- [x] Live Edit variants now carry machine-readable surface operations for structure, visual taste, and clarity/accessibility, with medium-specific plans for UI, book/page regions, image compositions, storyboard panels, and canvas objects rendered directly in the picked surface; canvas objects/regions and spatial Map objects now also show role-specific in-target variant treatments while cycling so the selected bounds visibly hot-swap instead of only showing a descriptive card
- [x] `Go` now creates a frame-bound `canvax-live-edit-request` with target type/selector/object id, normalized bounds, source frame, active design kit, transcript/text note, pins, strokes, current output binding, variants, and surface operations; Accept carries the same request into live/task/rewrite/checkpoint/preview-manifest and patch-task context
- [x] Asset candidate cards now expose `Pick`, create `generated-asset-candidate` Live Edit targets with the same canvas outline and image-composition variant operations, and bind accepted requests back onto the candidate plus its output slot
- [x] The Live Edit bottom bar now has an explicit `Draw` mode that sends output-backed targets to output correction marks, keeps same-canvas reply targets on the sketch canvas, and routes canvas/asset targets back to sketch-surface strokes
- [x] Same-canvas replies can be picked directly from the canvas, accept in-canvas comment pins, and capture nearby canvas strokes into the frame-bound Live Edit request before variants are generated
- [x] Sketch-canvas Pick mode can now click a drawn object/image region directly, or click blank canvas to create an arbitrary `canvas-region` target; accepted blank-region edits persist through `liveEditRegionBindings`
...
- [x] Accepted Live Edit targets with no explicit source file/task hint now emit `sourceSearchHints` and `sourceDiscovery: needs-source-search` in `codex-patch-task.json`, preserving selector, node-id, visible-text, label, and note queries so Codex can search the real project before treating the pick as screenshot-only
...
- [x] Local scratchpad Live Edit accepts with no preview URL now also save a non-renderable manifest binding, while Workbench/Preview target resolution still prefers renderable generated outputs for Output Focus; this keeps canvas regions, sketch objects, image regions, asset candidates, and Map objects durable without making the output pane look blank
- [x] Accepted local Live Edit bindings are now first-class rewrite queue entries even after the picker closes or the active target changes, so a scratch/object/region accept still reaches `canvax-rewrite-request-latest.*` instead of depending on the currently open picker state
- [x] Accepted Live Edit writebacks now carry the same target, accepted variant, original restore snapshot, request, pins, and writeback status into `artifacts/canvax/codex-output.json` when Codex publishes a bound output or patch result, so Workbench/Preview can recover the direct-edit binding from either manifest
...
- [x] Spatial Map objects now expose `Pick target`, become `spatial-map-object` Live Edit targets with an outline on the same Map card, accept direct comment pins and correction strokes, hot-swap role-specific variant treatments inside the Map card, and bind accepted requests back onto the Map object export/context
...
- [x] `npm run mcp` exposes those inspection payloads, including `get_host_handoff`, `create_task_pack`, `create_image_prompt_pack`, and `get_project_link`, through local stdio MCP tools for hosts that can register a local command
...
- [x] Codex in-app browser first operating docs for running the board, Preview, and generated app inside Codex
...
6. Use `docs/CODEX_BROWSER_WORKFLOW.md` as the preferred operator path for testing Canvax inside Codex in-app browser before building native embedding.
...
current baseline
  ...
  + Codex in-app browser first workflow
...
- The preferred manual validation path is now the Codex in-app browser rather than a separate external browser: invoke `/canvax` or run `./canvax --open-codex` to start or reuse the service and target `http://localhost:3210/` in the in-app editor, open Preview, inspect generated routes, and publish output context back through the Codex output manifest. External browser use is explicit through `./canvax --open-external` or `./canvax --chrome`.
- Workbench now surfaces the core choices that were previously too hidden: viewport/surface selection, new frame creation, connected section creation, free-canvas mode, local screen generation, generated-output correction marks, and an Open scratchpad rail/composer when the tray is intentionally collapsed. This is still not a true AI-native infinite canvas. It still cannot directly consume raw Codex chat microphone audio from the local web board, but Codex can now forward submitted chat transcripts through the local transcript bridge.
