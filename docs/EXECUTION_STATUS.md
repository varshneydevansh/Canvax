...
- [x] Live Edit variants now carry machine-readable surface operations for structure, visual taste, and clarity/accessibility, with medium-specific plans for UI, book/page regions, image compositions, storyboard panels, and canvas objects rendered directly in the picked surface
...
- [x] Spatial Map objects now expose `Pick target`, become `spatial-map-object` Live Edit targets with an outline on the same Map card, accept direct comment pins and correction strokes, hot-swap variants inside the Map card, and bind accepted requests back onto the Map object export/context
