...
The board self-test covers drawing tools, select/move/resize, eraser ink-layer behavior, Workbench dock brush sizing, Workbench action modes, Workbench Agent log rendering/activity, host/design-context handoff fields, flow link creation/deletion, task/image prompt packs, materialize, output activity, rewrite queue state, board-side rewrite execution, and large-session export consistency.
...
artifact, and Codex output manifest, then returns the current frame, spatial
workspace summary, design kit, output binding, and project-link binding in one
stable read-only payload. Keep it backward compatible: it is the local CLI shape
future MCP tools should mirror.
...
Use `npm run review-snapshot` after browser regression when the rendered board
or Preview screenshots need a local pixel-level smoke gate. The command samples
the latest browser snapshot index by default, writes
`exports/canvax-visual-snapshot-review-latest.{json,md}`, and flags blankness,
...
- browser regression captures `visualfixture=project-browser` on desktop and narrow screens
