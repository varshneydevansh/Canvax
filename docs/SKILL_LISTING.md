...
```text
Canvax is a Codex-first visual sketch companion. Use /canvax as the preferred slash-listed skill entry to start or reuse the local service and target the compact Canvax editor in Codex's right-side in-app browser. Draw rough UI screens, app flows, book spreads, storyboards, comic pages, posters, image directions, or visual notes; capture voice/text intent; then Canvax writes live exports, checkpoints, task packs, image prompt packs, and output manifests so Codex can build, rewrite, review, and bind generated work back to the same canvas without a separate OpenAI API key.
```
