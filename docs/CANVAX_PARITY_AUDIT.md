...
| True infinite spatial canvas | Map pan/zoom, edge expansion, trailing room, lasso, multi-select, groups, spatial objects, lightweight object property editing and per-type details, checkpoint history lane, context export | `web/app.js` `computeFlowSurfaceSize`, `ensureFlowWorkspaceMargin`, `buildSpatialWorkspaceExport`; browser self-test and docs | Still missing richer nested group editing, editable deep per-type object inspectors, editable history-lane controls, and inertial canvas feel |
