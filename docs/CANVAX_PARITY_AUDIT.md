...
| Unified Workbench / Advanced experience | Partial / stronger local | Shared dark workspace language, non-sticky solid Advanced command deck, shared Frame/Map/Preview data model, and a visible mode guide that maps Workbench to `Sketch` / `Talk` / `Make / Apply` and Advanced to `Project rail` / `Canvas deck` / `Handoff inspector`; docs explain mode boundary. | Advanced is still denser by design and still needs manual design review, but the shared hierarchy is now clearer. |
...
| Live sketch-and-voice rewrite loop | Partial / stronger local | Voice segments, transcript bridge, Apply/Live rewrite, rewrite request, local rewrite executor, output annotations with normalized changed-region bounds, output eraser deletion semantics, affected-region to component-target mapping, checkpoints, e2e workflow proof. | Not a continuous first-party Codex live co-edit loop; native Codex microphone and live ChatGPT/Codex host bridge require first-party integration. |
...
Verified locally on May 17, 2026 after the bounded Map viewport, `Tidy map`, Advanced non-sticky command deck, and preview-manifest compaction pass:
...
board browser self-test with 90 assertions, Preview browser self-test with 16
assertions, responsive board/Preview screenshots at 1440, 1024, 768, and 430
pixel widths, plus the Map viewport/tidy assertions added for this pass.
