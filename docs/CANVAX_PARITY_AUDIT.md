...
| More powerful for real implementation | Build/rewrite requests, local executors, React/framework handoff, component map, manifest publishing, output cards, Preview binding, `codex-patch-task.json`, `execute-patch`, and production-like manifest/token/review/patch proof | `scripts/execute-build-request.mjs`, `scripts/execute-rewrite-request.mjs`, `scripts/execute-patch-task.mjs`, `scripts/write-codex-output.mjs`, `scripts/production-port-proof.mjs`, `artifacts/canvax/codex-output.json`; `npm run e2e-workflow`; `npm run production-port-proof` | Real arbitrary production app edits still depend on Codex acting on the request, so this is not complete as a fully autonomous app builder |
