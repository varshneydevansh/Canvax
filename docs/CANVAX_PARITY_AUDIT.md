...
| More powerful for real implementation | Build/rewrite requests, local executors, React/framework handoff, component map, manifest publishing, output cards, Preview binding, `codex-patch-task.json`, and production-like manifest/token/review proof | `scripts/execute-build-request.mjs`, `scripts/execute-rewrite-request.mjs`, `scripts/write-codex-output.mjs`, `scripts/production-port-proof.mjs`, `artifacts/canvax/codex-output.json`; `npm run e2e-workflow`; `npm run production-port-proof` | Real production app edits still depend on Codex acting on the request, so this is not complete as a fully autonomous app builder |
...
| Live sketch-and-voice rewrite loop | Voice export, transcript bridge, rewrite request, Apply/Live rewrite, in-flight handoff queueing, output correction bounds, Preview tweak bounds, output eraser deletion semantics, changed region/component targeting, `codex-patch-task.json` with selectors/files/acceptance gates, build visual direction preservation, Codex port task preservation, checkpoints | `exports/canvax-voice-latest.md`, `exports/canvax-rewrite-request-latest.json`, `exports/canvax-preview-tweak-latest.json`, `scripts/execute-rewrite-request.mjs`, browser self-test, e2e rewrite proof | Continuous first-party Codex co-edit while sketching remains blocked on host integration; patch tasks are production-edit instructions, not automatic patches |
...
- live DOM-semantic tweak patching beyond the local Preview `Mark tweak`
  request and `codex-patch-task.json` file, where a selected output region
  becomes an automatically applied production edit
