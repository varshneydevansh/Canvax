...
| Generated variants as editable canvas objects | Strong local | `Create variants`, variant branch cards, matching `variant-branch` Map objects, `Use variant` directly in Map, `Make editable` output-preview branches, primary-state export through `spatialWorkspace.variantBranches`, `spatialWorkspace.variantBranches[].outputBinding`, and `spatialWorkspace.objects`; self-test covers variant creation, object rendering/export, Map action, output-preview to editable frame creation, promotion, and export. | Still not a semantic AI variant generator with a full per-property style inspector, but variants and output-edit branches are now editable spatial objects in the local Map. |
| Codex-built real app/screen generation with manifest binding | Partial / strong local | `Build with Codex` request, deterministic local executor, implementation bundle, frame-to-code ownership map, `artifacts/canvax/codex-output.json`, preview manifest binding, and `Make editable` from generated output preview to an `Output edit` frame preserving the generated target; `npm run e2e-workflow` verifies rough frame to build/rewrite manifest chain. | Real production implementation still depends on Codex acting on the request; local executor is a smoke/scaffold path, not a high-fidelity production generator. |
...
| True infinite spatial canvas | Map pan/zoom, edge expansion, trailing room, lasso, multi-select, selection-created groups, selecting group contents, fitting group bounds, ungrouping, spatial objects, front/back object layering, collapsible output shelf lane, object pinning, lightweight object property editing and per-type details, object focus filtering, collapsible checkpoint history lane, context export | `web/app.js` `computeFlowSurfaceSize`, `ensureFlowWorkspaceMargin`, `buildSpatialWorkspaceExport`; browser self-test and docs | Still missing richer nested group editing, editable deep per-type object inspectors, lane/timeline reorder controls, and inertial canvas feel |
| Generated variants as editable canvas objects | Variant branch frames and matching `variant-branch` Map objects with `Use variant`, primary state, and `Output edit` branches created from output preview cards | `web/app.js`, live export `spatialWorkspace.variantBranches`, browser self-test | Semantic AI variant generation and property-level style inspection remain future work |
| Codex-built real app/screen generation with manifest binding | Build request, local executor, preview artifact, implementation bundle, frame-to-code map, Codex output manifest, and editable output branch creation from a generated target | `exports/canvax-build-real-latest.json`, `scripts/execute-build-request.mjs`, `web/app.js`, `npm run e2e-workflow` | High-fidelity production code generation is not proven by the deterministic smoke scaffold |
...
Verified locally on May 17, 2026 after the output-preview to editable-frame pass:
