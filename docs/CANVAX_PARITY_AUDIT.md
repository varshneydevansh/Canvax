...
| True infinite spatial canvas | Map pan/zoom, minimap click-to-pan navigator, `Fit map` recovery, edge expansion, trailing room, lasso, multi-select, selection-created groups, selecting group contents, fitting group bounds, ungrouping, spatial objects, front/back object layering, collapsible output shelf lane, object pinning, lightweight object property editing and per-type details, object focus filtering, collapsible checkpoint history lane, context export | `web/app.js` `computeFlowSurfaceSize`, `ensureFlowWorkspaceMargin`, `renderFlowNavigator`, `onFlowNavigatorPointerDown`, `fitFlowMapToContent`, `buildSpatialWorkspaceExport`; browser self-test and docs | Still missing richer nested group editing, editable deep per-type object inspectors, lane/timeline reorder controls, and inertial canvas feel |
