...
| Live sketch-and-voice rewrite loop | Partial / stronger local | Voice segments, transcript bridge, Apply/Live rewrite, rewrite request, local rewrite executor, output annotations with normalized changed-region bounds, output eraser deletion semantics, Preview `Mark tweak` requests consumed as `source: preview-tweak` affected regions, affected-region to component-target mapping, rewrite-side `codex-patch-task.json` with suggested selectors/files/acceptance gates, `execute-patch` deterministic edits for Canvax-generated implementation bundles, attached build-contract `visualDirection` preservation for themed rewrite previews, attached Codex port task preservation for production-port context, in-flight Live rewrite queueing for newer handoffs, checkpoints, e2e workflow proof, and future host bridge boundary in `docs/CHATGPT_APP_BRIDGE.md`. | Not a continuous first-party Codex live co-edit loop; native Codex microphone and live ChatGPT/Codex host bridge require first-party integration, and arbitrary production app patches still need Codex judgment. |
...
| Open Design-style skill/design-system layer | Partial / stronger visible | Root `DESIGN.md` detection, starter `DESIGN.md` generation, a visible `Design kit` summary in Workbench/Advanced, built-in presets, searchable repository JSON kits under `design-kits/`, `npm run validate-design-kits`, `npm run validate-design-kits -- --query <term>`, `npm run package-design-kits` for versioned/checksummed library export, local current-frame sketch token extraction, locally readable placed/reference-image color sampling, `npm run extract-tokens` for URL/file/generated-artifact CSS, static HTML/JSX `semanticStructure`, `npm run review-artifact` static artifact design review, local screenshot/image palette extraction, `npm run review-snapshot` for local pixel-level browser screenshot review, `npm run review-dom` for local rendered Preview DOM/layout review, `npm run review-jury` for a local design-jury verdict, Preview `Mark tweak` and `/api/save-preview-tweak` for no-API output-region correction requests, rewrite-side `codex-patch-task.json` with selectors/files/acceptance gates, `npm run execute-patch` generated-bundle patch proof, Advanced `Import external` for the latest external token pack, `npm run verify-tokens` for local implementation artifact palette enforcement and manifest-listed production file enforcement, `npm run production-port-proof` for local route/component/CSS proof of that gate plus production-like patch-task execution, `npm run project-link` for explicit local app route/component/CSS frame binding, `npm run inspect` for a local read-only current-frame/spatial-workspace/design-kit/output-binding/project-link bridge, `npm run mcp` for read-only stdio MCP tools including `get_project_link`, no-API style locks, semantic variant recipes, image prompt packs, and Codex build/rewrite handoff contracts that now export the active `designKit` source, preset, recipe, action mode, board mood, surface, frame notes, style knobs, and extracted sketch/reference-token cues. Open Design is tracked as an external reference in `docs/STITCH_GAP_ROADMAP.md`, including skill/design-system libraries, first-turn discovery, direction picking, sandboxed artifact preview, MCP-style live file access, design-jury critique, live preview tweak targeting, and code-folder linking as explicit benchmark ideas. | Canvax now has a starter file-based kit gallery, kit search, CLI kit query, versioned kit-library packaging, static artifact semantic extraction/review, local screenshot palette extraction/review, local rendered DOM/layout review, local design-jury aggregation, local Preview region-tweak requests, a Codex patch-task handoff, generated-bundle applied-patch proof, a production-like port/patch proof fixture, a local project-link bridge, a local read-only CLI bridge, and a local stdio MCP server, but not hosted/team kit-library sharing, native Codex/ChatGPT host registration, hosted AI browser visual critique, automatic arbitrary production patching, or proof from Codex completing a real user production port. |
...
| More powerful for real implementation | Build/rewrite requests, local executors, React/framework handoff, component map, manifest publishing, output cards, Preview binding, `codex-patch-task.json`, `execute-patch`, `project-link`, and production-like manifest/token/review/patch proof | `scripts/execute-build-request.mjs`, `scripts/execute-rewrite-request.mjs`, `scripts/execute-patch-task.mjs`, `scripts/write-codex-output.mjs`, `scripts/production-port-proof.mjs`, `scripts/link-project-target.mjs`, `artifacts/canvax/codex-output.json`; `npm run e2e-workflow`; `npm run production-port-proof`; `npm run project-link` | Real arbitrary production app edits still depend on Codex acting on the request, so this is not complete as a fully autonomous app builder |
...
| Codex-built real app/screen generation with manifest binding | Build request, local executor, preview artifact, implementation bundle, React/framework handoff, frame-to-code map, Codex port task, Codex output manifest, editable output branch creation from a generated target, request/executor target binding, context-aware starter theming from variant/style/Map guidance, a local project-link bridge, and a production-like route/component/CSS/token/review/patch proof fixture | `exports/canvax-build-real-latest.json`, `scripts/execute-build-request.mjs`, `scripts/execute-rewrite-request.mjs`, `scripts/execute-patch-task.mjs`, `scripts/production-port-proof.mjs`, `scripts/link-project-target.mjs`, `web/app.js`, `npm run e2e-workflow`, `npm run production-port-proof`, `npm run project-link` | High-fidelity production code generation is not proven by the deterministic smoke scaffold, local fixture, or project-link bridge alone |
| Live sketch-and-voice rewrite loop | Voice export, transcript bridge, rewrite request, Apply/Live rewrite, in-flight handoff queueing, output correction bounds, Preview tweak bounds, output eraser deletion semantics, changed region/component targeting, `codex-patch-task.json` with selectors/files/acceptance gates, `execute-patch` generated-bundle edit proof, build visual direction preservation, Codex port task preservation, checkpoints | `exports/canvax-voice-latest.md`, `exports/canvax-rewrite-request-latest.json`, `exports/canvax-preview-tweak-latest.json`, `scripts/execute-rewrite-request.mjs`, `scripts/execute-patch-task.mjs`, browser self-test, e2e rewrite proof | Continuous first-party Codex co-edit while sketching remains blocked on host integration; generated-bundle patching is proven, but arbitrary production app patches still need Codex |
...
Verified locally on May 19, 2026 after the project-link bridge, production-like patch proof, React/framework build handoff, no-API image generation brief, no-API image host task, and strict prompt-to-artifact goal-audit passes:
...
- real-project evidence beyond the local production-like fixture that
  manifest-listed token enforcement survives a Codex production port into
  arbitrary app files
...
- live DOM-semantic tweak patching beyond the local Preview `Mark tweak`
  request, `codex-patch-task.json`, and generated-bundle `execute-patch` proof,
  where a selected output region becomes an automatically applied edit in an
  arbitrary production app
