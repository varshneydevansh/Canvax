...
- first-class MCP/native host registration for the read-only Canvax bridge so
  Codex or ChatGPT can call current-frame/design-kit/spatial-workspace/output
  tools directly instead of via the local `npm run inspect` CLI
