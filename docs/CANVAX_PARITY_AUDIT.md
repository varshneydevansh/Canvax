...
| Open Design-style skill/design-system layer | Partial / stronger visible | Root `DESIGN.md` detection, starter `DESIGN.md` generation, a visible `Design kit` summary in Workbench/Advanced, no-API style locks, semantic variant recipes, image prompt packs, and Codex build/rewrite handoff contracts that now export the active `designKit` source, recipe, action mode, board mood, surface, frame notes, and style knobs. Open Design is tracked as an external reference in `docs/STITCH_GAP_ROADMAP.md`. | Canvax does not yet have a browsable design-system/skill gallery like Open Design, visual token extraction from URLs/screenshots, or strict token enforcement in generated outputs. |
