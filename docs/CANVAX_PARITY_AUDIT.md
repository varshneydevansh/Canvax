...
| True infinite spatial canvas | Partial | Workbench `Map`, pan/zoom, frame cards, variant cards, spatial objects, selected-object actions, group movement, group duplication, context export in `web/app.js`; docs in `docs/STITCH_GAP_ROADMAP.md`. | Not a full arbitrary-object infinite canvas yet: no lasso selection for Map objects, no multi-object Map transform box, no nested group inspector, no inertial canvas feel, no history lanes. |
