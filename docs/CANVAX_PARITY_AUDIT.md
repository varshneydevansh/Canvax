...
| Open Design-style skill/design-system layer | Partial | Root `DESIGN.md` detection, starter `DESIGN.md` generation, no-API style locks, semantic variant recipes, image prompt packs, and Codex build/rewrite handoff contracts. Open Design is tracked as an external reference in `docs/STITCH_GAP_ROADMAP.md`. | Canvax does not yet have a browsable design-system/skill gallery like Open Design; the next step is to make design rules and recipes easier to discover/apply without cluttering Workbench. |
