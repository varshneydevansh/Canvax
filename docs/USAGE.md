...
Use `Extract tokens` when the current sketch itself should become part of the design contract. Canvax samples drawn non-eraser elements and records a local token summary: dominant colors, element mix, density, shape language, text cues, and asset-slot cues. Those tokens export through `designKit` and feed the no-API style lock used by image prompt packs, asset candidates, and Build-with-Codex context. This is local current-frame extraction; it does not yet scrape tokens from URLs, screenshots, or external apps.
...
- The local executor reads the active Design kit too. Product, poster, book/storyboard, and dashboard kits can shape the generated starter theme/atmosphere, and extracted sketch tokens can carry palette/density/shape cues into the integration contract and Codex port task.
