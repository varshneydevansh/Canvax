...
`Free canvas` is still a large single-frame scratchpad preset. `Map` is the first persistent spatial project layer. It is not yet a full infinite canvas with native trackpad pan/zoom and grouped areas, but it now gives Canvax a zoomable spatial memory for frames, variant branches, manual notes, reference files/images, asset candidates, generated outputs, artifacts, and workspace changes.
