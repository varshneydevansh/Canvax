...
- `scripts/execute-build-request.mjs`: a deterministic local executor that turns the latest build request into a frame-bound preview artifact, `implementation/` bundle, React-ready `CanvaxScreen.jsx`/CSS handoff, Vite/Next adapter stubs, `canvax-component-map.json` ownership map, and published `artifacts/canvax/codex-output.json`
