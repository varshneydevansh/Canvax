...
manual Map controls
  -> labeled group regions, note cards, direct pasted/dropped text notes, and reference file/image cards
  -> dropped file/image cards at pointer position
  -> pasted image cards in the visible Map region
  -> state.spatialObjects
...
`visualDirection.designTokens.palette`, then checks generated implementation
CSS/HTML/JSX files for those token colors. This keeps the Open Design-style
design-system loop inspectable: extraction creates token artifacts, Build with
...
inspection bridge into a designer-facing verdict with categories for hierarchy,
accessibility, responsive readiness, brand/system fit, preview tweak targeting,
