...
- Better timeline/history navigation for long sessions. **Initial output shelf and checkpoint history lanes now ship as visible Map lanes; the checkpoint lane has Hide/Show control, object focus filters can isolate history from output/assets/notes, pinned objects stay visible across focus/collapse, and exported state includes `spatialWorkspace.lanes[]`, `spatialWorkspace.lanes[].collapsed`, `spatialWorkspace.objectFilter`, and per-object `pinned`; richer reorder controls remain open.**
