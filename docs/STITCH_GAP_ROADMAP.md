...
- Browser regression now includes deterministic `visualfixture=workbench-agent-log` and `visualfixture=advanced-map` states. The Workbench fixture screenshots the compact open Agent log on desktop and narrow screens, while the Advanced fixture screenshots Advanced Map at desktop and tablet sizes and checks that the command deck stays opaque while generated-output cards remain designer-readable.
