...
- Advanced mode keeps the full frame, flow, manifest, capture, and inspector surface, but now uses the same dark dotted Canvax visual system as Workbench. The mode switch and deck labels make it read as a technical inspector layer for the same workbench rather than a different app, and its command deck now uses a sticky opaque inspector surface so canvas/grid content does not bleed through the controls while scrolling.
