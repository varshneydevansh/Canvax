...
- Impeccable-style direct target editing. **Initial Workbench `Live Edit` / `Pick target` slice shipped: generated outputs, same-origin preview DOM elements, and selected canvas/image objects can become frame-bound live edit targets, the chosen bounds are outlined, the bottom picker bar captures action/note/comment/variant state, comment pins can be placed and dragged directly on the target, `Go` creates targeted variants that hot-swap in the same surface, and each variant now carries visible, machine-readable surface operations for structure, visual taste, and clarity/accessibility across UI, book/page, image, storyboard, and canvas-object targets. `Go` also creates a frame-bound `canvax-live-edit-request` with target selector/object id, normalized bounds, source frame, active design kit, transcript/text note, pins, strokes, current output binding, variants, and operations. `Accept` persists that request into live/task/rewrite/checkpoint/preview-manifest and patch-task handoffs where relevant, and `Escape`/`Close` discards unaccepted picks. Comment pins and heuristic stroke semantics, including component circles and multi-stroke groups, export with the target. Same-origin DOM picks now also preserve explicit source/task hints such as `data-canvax-source-file`, `data-canvax-source-symbol`, and `data-canvax-task-id`; accepted picks thread those hints into rewrite/patch tasks; `execute-patch` can update local source-hinted component/CSS files plus append hinted task notes; and output-backed Accept saves a post-writeback checkpoint with rewrite/patch status. Arbitrary unhinted source discovery remains open.**
