...
- Infinite canvas with pan/zoom. **Initial Workbench Map drag-pan, Shift-drag lasso selection, selected-set dragging/resizing, cursor-centered pinch/ctrl-wheel zoom, edge expansion when cards/objects are dragged into the left/top boundary, persistent trailing workspace room, movable/resizable labeled group regions that can move contained cards/objects with exported containment, and manual note/reference, asset-candidate, output-preview, output-file, and code-change spatial objects are shipped; generated output cards now infer frame binding from artifact paths, hide outputs bound only to deleted frames, collapse repeated outputs to the latest useful per-frame/per-kind card, and sit inside the output shelf lane. Richer nested editing remains open.**
