...
- Tracks attached asset candidate previews with select and accept actions, so image-generation choices become explicit local handoff state.
...
- Adds `Build code` / `Build with Codex`, which writes a no-API frame-to-code request and immediately runs the local build executor so Workbench and Preview get a frame-bound preview plus implementation starter bundle before Codex replaces or ports it into real app/page files.
