...
- Adds a visible Advanced `Design kit` card with active rule sources, local kit presets for product apps, poster systems, book spreads, dashboards, and storyboards, plus `Extract tokens` for deriving palette, density, shape language, and element mix from the current sketch without an API. Applying a kit updates the board surface, mood, action mode, generation recipe, and empty frame notes without touching the sketch.
